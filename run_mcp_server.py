#!/usr/bin/env python3
"""
MCP Server Runner with Complete Output Suppression
"""
...
# Temporarily redirect stdout during imports
original_stdout = sys.stdout
sys.stdout = NullWriter()

try:
    # Import everything while stdout is suppressed
    import asyncio
    import json
    import tempfile
    import logging
...
async def handle_list_tools() -> List[Tool]:
    ...
    return [
        ...
        Tool(
            ...
            description="Update local dumps of preprint servers",
            inputSchema={
                "type": "object",
                "properties": {
                    "servers": {
...
async def search_pubmed(arguments: Dict[str, Any]) -> List[TextContent]:
    ...
    try:
        get_and_dump_pubmed_papers(query, output_filepath=temp_file, retmax=max_results)
...
async def search_arxiv(arguments: Dict[str, Any]) -> List[TextContent]:
    ...
    try:
        get_and_dump_arxiv_papers(query, output_filepath=temp_file, max_results=max_results)
        results = load_jsonl(temp_file)
